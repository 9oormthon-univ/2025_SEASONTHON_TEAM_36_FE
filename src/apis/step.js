...
/** [PUT] Step 수정 */
export async function modifyStep(stepId, description) {
  ...
  try {
    ...
    return data; // { todoId, todoTitle, stepId, stepDate, description, isCompleted }
  } catch (err) {
    const status = err?.response?.status;
    if (status === 403) throw new Error('Step에 접근할 권한이 없습니다.');
    if (status === 404) throw new Error('Step을 찾을 수 없습니다..');
    throw err;
  }
}
...
export async function deleteStep(stepId) {
  if (stepId == null) throw new Error('Step 수정을 위해 stepId가 필요합니다.');
  try {
    const { data } = await mainApi.delete(`${BASE}/${stepId}`, {
      headers: { Accept: 'application/json' },
    });
    return data; // { todoId, todoTitle, stepId, stepDate, description, isCompleted }
  } catch (err) {
